#!/usr/bin/env python3
"""timer-wired-deploy.py — Deploy per-exercise rest timers across all programs"""
...
def push_file(local, repo):
    ...
    payload = {"message": "feat: rest timer " + repo, "content": content, "branch": BRANCH}
    if sha:
        payload["sha"] = sha
    r, st = api_request("PUT", API + "/" + repo, payload)
    if st in (200, 201):
        print("  OK  " + action + "  " + repo)
        return True
    print("  FAIL  " + repo + " -- " + str(st) + ": " + r.get("message", ""))
    return False

def main():
    if GITHUB_TOKEN == "YOUR_PERSONAL_ACCESS_TOKEN_HERE":
        print("")
        print("Set your GITHUB_TOKEN first.")
        print("Go to: https://github.com/settings/tokens")
        print("")
        sys.exit(1)
    print("")
    print("Deploying " + str(len(FILES)) + " files to " + REPO_OWNER + "/" + REPO_NAME)
    print("")
