...
(function () {
  ...
  // actually loaded (vs a stale cached copy). Only shows on pages that really
  // have a superset/triset to hop, so it isn't noise everywhere. Safe to
  // remove after the rollout is confirmed.
  function flashReady() {
    if (!document.querySelector('.ss-ex[data-type="ssex"]')) return;
    if (document.getElementById('sshop-ready')) return;
    var t = document.createElement('div');
    t.id = 'sshop-ready';
    t.textContent = '⚡ Superset hop active';
    t.style.cssText = 'position:fixed;left:50%;top:calc(10px + env(safe-area-inset-top));' +
      'transform:translateX(-50%);z-index:2000;background:rgba(168,85,247,0.96);color:#fff;' +
  ...
  // Give inline renderers a moment to build the cards before we check.
  function maybeFlash() { setTimeout(flashReady, 1200); }
  if (document.body) maybeFlash();
  else document.addEventListener('DOMContentLoaded', maybeFlash);
