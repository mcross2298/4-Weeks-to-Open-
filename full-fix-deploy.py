#!/usr/bin/env python3
"""
full-fix-deploy.py — Deploy all fixed workout files
Fixes: timers (11 files), workout summaries (37 files), JS syntax errors (3 files)
"""
...
def push(local,repo):
    ...
    payload={"message":f"fix: {repo}","content":content,"branch":BRANCH}
    if s: payload["sha"]=s
...
def main():
    ...
    print(f"\n🚀 Full fix deploy — {len(FILES)} files → {REPO_OWNER}/{REPO_NAME}\n")
